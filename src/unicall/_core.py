    ...
    def _make_done_callback(
        ...
        def _on_done(finished: asyncio.Task[T]) -> None:
            if not finished.cancelled():
                finished.exception()
            if self._ttl:
                # Keep the finished flight around a little longer: a burst
                # that arrives within `ttl` seconds of completion reuses the
                # cached result instead of paying for a brand new flight.
                asyncio.get_event_loop().call_later(self._ttl, self._evict, key, finished)
            else:
                self._evict(key, finished)

        return _on_done

    def _evict(self, key: Hashable, task: asyncio.Task[T]) -> None:
...
def unicall(
    *, key: KeyFunc[P] | None = None, ttl: float | None = None
) -> Callable[[Callable[P, Awaitable[T]]], Coalescer[P, T]]:
    """Decorate an async function so concurrent calls with equal arguments
    (or an equal `key(...)`) share a single execution. With `ttl`, a
    finished flight is kept for `ttl` seconds so a burst right after
    completion reuses it instead of starting a new one.
    """

    def decorator(func: Callable[P, Awaitable[T]]) -> Coalescer[P, T]:
        return Coalescer(func, key=key, ttl=ttl)
